...
function initTheside() {
  ...
  function videoint() {
    ...
    $(".video-container").css(
      "height",
      Number((720 / 1280) * $(window).width()) + "px"
    );
    if ($(".video-container").height() < $(window).height()) {
      $(".video-container").css("height", $(window).height() + "px");
      $(".video-container").css(
        "width",
        Number((1280 / 720) * $(window).height()) + "px"
      );
    }
  }
  videoint();
  ...
  $(".top_fix-filter").scrollToFixed({
    ...
    limit: function () {
      var a =
        $(".limit-box").offset().top - $(".top_fix-filter").outerHeight(true);
      return a;
    },
  });
  var mouse = {
  ...
  if ($("#map-single").length > 0) {
    var latlog = $("#map-single").data("latlog"),
      ...
      map = L.map("map-single").setView(latlog, 30);
